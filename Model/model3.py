...
from tensorflow.keras.layers import Dense, Dropout, GlobalAveragePooling2D, GlobalAveragePooling1D, Concatenate, Input, Lambda
...
DATASET_PATH = "Data"
MODEL_DIR = "Model"
PARAMS_PATH = os.path.join(MODEL_DIR, "best_params_cnn_vit.json")
SELECTED_CLASSES = [
...
TUNER_LOG_DIR = os.path.join("tuner_logs", f"cnn_vit_{timestamp}")
...
def build_model(hp):
    ...
    # CNN branch
    cnn_base = MobileNetV3Large(input_tensor=input_tensor, weights='imagenet', include_top=False)
    cnn_base.trainable = False
    cnn_output = GlobalAveragePooling2D()(cnn_base.output)

    # ViT branch
    vit_input = Lambda(vit_preprocess, name='vit_preprocess')(input_tensor)
    vit_input = Lambda(lambda x: tf.transpose(x, [0, 3, 1, 2]), name='vit_transpose')(vit_input)
    vit_output = vit_model({"pixel_values": vit_input})[0]
    vit_output = GlobalAveragePooling1D()(vit_output)

    # Merge branches
    merged = Concatenate()([cnn_output, vit_output])
    merged = Dropout(hp.Float("dropout", 0.2, 0.5, step=0.1))(merged)
    merged = Dense(hp.Int("dense_units", 128, 512, step=128), activation='relu')(merged)
    final_output = Dense(len(SELECTED_CLASSES), activation='softmax')(merged)
...
if os.path.exists(PARAMS_PATH):
    ...
    with open(PARAMS_PATH, 'r') as f:
        saved = json.load(f)
    best_model = tf.keras.models.load_model(os.path.join(MODEL_DIR, "cnn_vit_sign_language_model.keras"))
    ...
    best_model.save(os.path.join(MODEL_DIR, "cnn_vit_sign_language_model.keras"))
...
METRICS_PATH = os.path.join(MODEL_DIR, "training_metrics_cnn_vit.json")
...
print(f"\nModel saved at: {os.path.join(MODEL_DIR, 'cnn_vit_sign_language_model.keras')}")
