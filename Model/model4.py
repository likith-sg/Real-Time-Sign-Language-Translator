...
from tensorflow.keras.layers import Input, Dense, Dropout, LSTM, Reshape, Concatenate, GlobalAveragePooling2D, Bidirectional, BatchNormalization
...
DATASET_PATH = "Data"
MODEL_DIR = "Model"
PARAMS_PATH = os.path.join(MODEL_DIR, "best_params_cnn_lstm.json")
SELECTED_CLASSES = [
...
TUNER_LOG_DIR = os.path.join("tuner_logs", f"cnn_lstm_{timestamp}")
...
def build_model(hp):
    ...
    lstm = Bidirectional(LSTM(hp.Int("lstm_units", 128, 512, step=64), return_sequences=True))(lstm_input)
    lstm = LSTM(hp.Int("lstm_units_2", 64, 256, step=64), return_sequences=True)(lstm)
    lstm = LSTM(hp.Int("lstm_units_3", 64, 256, step=64), return_sequences=False)(lstm)
    lstm = BatchNormalization()(lstm)

    x = Dropout(hp.Float("dropout", 0.3, 0.5, step=0.1))(lstm)
    x = Dense(hp.Int("dense_units", 128, 512, step=128), activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
...
if os.path.exists(PARAMS_PATH):
    ...
    with open(PARAMS_PATH, 'r') as f:
        saved = json.load(f)
    best_model = tf.keras.models.load_model(os.path.join(MODEL_DIR, "cnn_lstm_model.keras"))
    ...
    with open(PARAMS_PATH, 'w') as f:
        json.dump({
            ...
            "lstm_units_3": best_hp.get('lstm_units_3'),
            "timestamp": timestamp,
            "total_time_sec": total_time
        }, f, indent=2)

    best_model.save(os.path.join(MODEL_DIR, "cnn_lstm_model.keras"))
...
METRICS_PATH = os.path.join(MODEL_DIR, "training_metrics_cnn_lstm.json")
...
print(f"\nModel saved at: {os.path.join(MODEL_DIR, 'cnn_lstm_model.keras')}")
