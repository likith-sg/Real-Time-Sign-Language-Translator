...
DATASET_PATH = "Data"
MODEL_DIR = "Model"
PARAMS_PATH = os.path.join(MODEL_DIR, "best_params_resnet50.json")
SELECTED_CLASSES = [
...
TUNER_LOG_DIR = os.path.join("tuner_logs", f"tune_{timestamp}")
...
def build_model(hp):
    ...
    resnet_base.trainable = False  # Freeze the ResNet50 layers
    
    resnet_output = GlobalAveragePooling2D()(resnet_base.output)
    
    # Add dropout and dense layers
    x = Dropout(hp.Float("dropout", 0.2, 0.5, step=0.1))(resnet_output)
...
if os.path.exists(PARAMS_PATH):
    ...
    with open(PARAMS_PATH, 'r') as f:
        saved = json.load(f)
    best_model = tf.keras.models.load_model(os.path.join(MODEL_DIR, "resnet50_sign_language_model.keras"))
else:
    ...
    best_model.save(os.path.join(MODEL_DIR, "resnet50_sign_language_model.keras"))
...
METRICS_PATH = os.path.join(MODEL_DIR, "training_metrics_resnet50.json")
...
print(f"\n Model saved at: {os.path.join(MODEL_DIR, 'resnet50_sign_language_model.keras')}")
